...
/* the 3th proess helper function that is located here because
   it makes it a lot easier. it initializes the process table, setting all
   states to RUNNING*/
void process_init(){
  ...
  // Attempt find a free spot for the process
  for (int i=0; i < PROCESS_MAX_PROCESSES; i++) {

    // If a free spot is found, set state to running and set PID
    process_table[i].state = STATE_FREE;
