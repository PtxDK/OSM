  ...
  if (value >= 0) {
    for (int i = 0; i < MAX_USR_SEM; i++) {
      ...
      if (usr_sem_table[i].inuse == 0) {
        stringcopy((char*)name, usr_sem_table[i].name, 32);
...
// Try to remove/delete a semaphore (a wrapper)
int usr_sem_destroy(usr_sem_t* sem) {
  ...
  // Wrap the destruction Baybyyyyyyyyyyyyyyyyyyyyyyyyy
