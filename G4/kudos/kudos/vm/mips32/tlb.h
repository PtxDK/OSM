...
//helper functions
uint32_t tlb_entry_is_valid(tlb_entry_t*, uint32_t);
void tlb_error(uint32_t, char*);
void tlb_access_exception(uint32_t);


/* exception handlers */
void tlb_modified_exception(uint32_t);
void tlb_load_exception(uint32_t);
void tlb_store_exception(uint32_t);

/* Forward declare pagetable_t (== struct pagetable_struct_t) */
struct pagetable_struct_t;
// void tlb_fill(struct pagetable_struct_t *pagetable); not used anymore
