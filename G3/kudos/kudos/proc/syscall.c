...
// All the Wrapperz
usr_sem_t* syscall_usr_sem_open(const char* name, int value) {
  return usr_sem_open(name, value);
}

int syscall_usr_sem_destroy(usr_sem_t* sem) {
  return usr_sem_destroy(sem);
}

int syscall_usr_sem_p(usr_sem_t* sem) {
  return usr_sem_p(sem);
}

int syscall_usr_sem_v(usr_sem_t* sem) {
  return usr_sem_v(sem);
}
                        ...
                        uintptr_t arg0, uintptr_t arg1, uintptr_t arg2)
{
  ...
  case SYSCALL_USR_SEM_DESTROY:
    usr_sem_destroy((usr_sem_t*) arg0); // cast to usr_sem_t* mby?
    break;
